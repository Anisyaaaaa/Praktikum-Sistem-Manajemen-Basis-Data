...
UPDATE produk SET Harga = 60000 WHERE id_produk = 1;
UPDATE produk SET Harga = 47500 WHERE id_produk = 2;
select * from produk;
...
UPDATE transaksi SET Jumlah = 3 WHERE id_transaksi = 1;
UPDATE transaksi SET total_harga = 180000 WHERE id_transaksi = 1;
UPDATE transaksi SET jumlah = 4 WHERE id_transaksi = 2;
UPDATE transaksi SET total_harga = 190000 WHERE id_transaksi = 2;
select * from transaksi;
INSERT INTO Transaksi (ID_Transaksi, ID_Produk, ID_Supplier, ID_Karyawan, Jumlah, Total_Harga, Tanggal_Transaksi) VALUES
	(1, 1, 1, 1, 50, 7500000, '2024-05-03 09:00:00'),
	(2, 2, 2, 2, 40, 10000000, '2024-05-04 09:15:00'),
...
BEGIN
    SELECT CONCAT('Produk ', Produk.Nama_Produk, ' Berada di ', Gudang.Nama, ' Selama ', DATEDIFF(CURDATE(), Stok.Tanggal_Update), ' Hari.') AS Result
    FROM Produk
...
BEGIN
	SELECT * FROM Transaksi
	WHERE Transaksi.Tanggal_Transaksi >= Batas_Waktu 
    AND Transaksi.Tanggal_Transaksi < CURDATE()
    AND Total_Harga > 200000;  
END//
DELIMITER ;

CALL Hapus_Transaksi('2024-05-12');
